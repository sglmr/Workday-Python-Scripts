...
file_1_name = 'file 1.csv'
file_2_name = 'file 2.csv'
results_file_name = 'Compare Results.csv'
key = 'Employee Id'

# Get Paths to files
file_1_path = os.path.join(sys.path[0], file_1_name)
file_2_path = os.path.join(sys.path[0], file_2_name)
results_file_path = os.path.join(sys.path[0], results_file_name)
...
for f1_row in file_1:
    for f2_row in file_2:
        if f1_row[key] != f2_row[key]: # Cycle through data keys until employee is found
            continue
        for col in columns:
